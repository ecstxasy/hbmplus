...
// mf i am not waiting that long for that.. add better coolants or do what i did the amount of time it takes to cool down is unironically unplayable

public class TileEntityMegaCooler extends TileEntityWasteDrum {
				...
				for(int i = 0; i < 12; i++) {
					...
					if(slots[i] != null) {
						...
						if(slots[i].getItem() instanceof ItemRBMKRod) {
							...
							rod.updateHeat(worldObj, slots[i], 0.9D);
							rod.provideHeat(worldObj, slots[i], 20D, 0.9D);
